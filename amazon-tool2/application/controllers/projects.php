...
class Projects extends CI_Controller {
	...
	public function progress_dashboard($project_id){
		...
		//1. collect best selling page ursl
		//$this->collect_best_selling_page_urls($project_id);
		
		$data['project_id'] = $project_id;
		$this->load->view('projects/progress_dashboard',$data);
					...
					if(!$this->project_model->is_exist($save_data)){
						...
						if($result>0){
							$form_data['gen_message'] = array(
									'type' => 'success',
									'text' => 'Data saved!');
							//$this->redirect_home(site_url('best_selling_urls/progress_dashboard/'.$this->new_project_id));
							$this->redirect_home(site_url('projects/project_init_dashboard/'.$this->new_project_id));
						}else{
			...
			if($validation){
				...
				if($result>0){
					$form_data['gen_message'] = array(
							'type' => 'success',
							'text' => 'Data updated!');				
					$this->redirect_home(site_url('projects/project_init_dashboard/'.$id));
	...
	*/
	...
	public function produce_grid_feed($limit=25,$offset=0){
		...
		foreach($record_set['result_set'] as $record ){
			...
			$xml->addNode('cell','<a href="'.
					site_url('projects/overview/'.$record['project_id']).'" >Overview</a> '.
					'<a href="'.
	...
	public function redirect_home($url){	
	...
	public function project_init_dashboard($project_id){
		...
		$this->load->view('projects/init_dashboard',$data);
	...
	public function get_product_QA($asin_code ,$project_id){
		...
		try{
			...
			if(!empty($divQnA)){			
				...
				foreach($divQnA as $item){						
					$q[$ind]['q'] = @$item->children(0)->children(0)->plaintext;
					$q[$ind]['a'] = @(isset($item->children(1)->children(0)->children(1)->plaintext))?
					$item->children(1)->children(0)->children(1)->plaintext:
					$item->children(1)->children(0)->plaintext;				
					$ind++;	
				};//end foreach	
				...
				//if there are more than 1 QnA pages
				if($noOfAs >0){
					...
					for($i=2;$i<=8;$i++){
						...
						foreach($divQnA as $item){					
							...
							$q[$ind]['q'] = @$item->children(0)->children(0)->plaintext;
							$q[$ind]['a'] = @(isset($item->children(1)->children(0)->children(1)->plaintext))?
								$item->children(1)->children(0)->children(1)->plaintext:$item->children(1)->children(0)->plaintext;								
							$ind++;
				...
				$q[$ind]['q'] = "No content found!";
				$q[$ind]['a'] = "No content found!";
				$ind++;		
			}//end if else
		...
		//endter collected data into table
		foreach($q as $d){
			
			$dup_data = array(
					'project_id' => $project_id,
					'ASIN_id' => $asin_code,
					'question' => htmlspecialchars(trim($d['q'])),
					'answer' => htmlspecialchars(trim($d['a']))
			);
			...
			if(!$this->product_qna_model->is_exist($dup_data)){
				$this->product_qna_model->save($save_data);
			}	
			
		}//end foreach

		//update product table by setting qna fetched = 1
		$update_data = array('qna_fetched' => 1	);
		$this->products_model->update_record(
				$update_data,
				array(
						'project_id' => $project_id,
						'ASIN_id' => $asin_code
	...
	public function prepare_product_contents($asin_code,$project_id){

		

		$record_set = $this->products_model->select_records('*',null,null,
				array('project_id'=>$project_id,'asin_id' => $asin_code)
		);
		
		//get product qna
		$qna_record_set = $this->product_qna_model->select_records('*',null,null,
				array('project_id'=>$project_id,'asin_id' => $asin_code)
		);
		
		//var_dump($qna_record_set);exit;
		$product = $record_set['result_set'][0];
		...
		//prepare qna contents
		foreach($qna_record_set['result_set'] as $qna){
			$content .= "<p>".
				"<b>Question:</b>".
				"<br>".htmlspecialchars_decode($qna['question']).
				"</p>".
				"<p>".
				"<b>Answer:</b>".
				"<br>".
				htmlspecialchars_decode($qna['answer']).
				"</p>".
				"<br>";
				
		}
		
		//var_dump($product_data_array->Items->Item);
		
		$data['result'] = $product_data_array;
		$data['asin_code'] = $asin_code;
		$data['project_id'] = $project_id;
		$data['contents'] = $content;
		
		$this->load->view('projects/prepare_content',$data);
	...
	public function get_product_QA($asin_code,$project_id){
		...
		//$record_set = $this->products_model->select_records('id,project_id,ASIN_id',null,null,array('project_id'=>$project_id));
			...
			try{
				...
				if(!empty($divQnA)){
					...
					foreach($divQnA as $item){
						...
						$as = @(isset($item->children(1)->children(0)->children(1)->plaintext))?
								$item->children(1)->children(0)->children(1)->plaintext:
								$item->children(1)->children(0)->plaintext;
						
						
						//var_dump(@$item->children(0)->children(0)->plaintext."<br>answers".$item->children(1)->children(0)->plaintext);exit;	
	
						$save_data = array(
								'project_id' => $project_id,
								'ASIN_id' => $asin_code,
								'question' => trim($q),
								'answer' => htmlspecialchars(trim($as))
						);
					...
					for($i=2;$i<=8;$i++){
						...
						foreach($divQnA as $item){
							...
							$q = @$item->children(0)->children(0)->plaintext;
							//$as = @$item->children(1)->children(0)->children(1)->plaintext;
